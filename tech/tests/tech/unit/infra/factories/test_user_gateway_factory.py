# tests/unit/infra/factories/test_user_gateway_factory.py - Versão corrigida
import pytest
...
class TestUserGatewayFactory:
    """Unit tests for the UserGatewayFactory."""
    ...
    def test_create_with_none_resilience_mode(self, monkeypatch):
        ...
        monkeypatch.setenv("USER_GATEWAY_RESILIENCE", "circuit_breaker")
        ...
        with patch.dict('sys.modules', {
            'tech.infra.gateways.circuit_breaker_user_gateway': Mock(CircuitBreakerUserGateway=circuit_breaker_mock),
            'tech.infra.gateways.http_user_gateway': Mock(HttpUserGateway=http_mock)
        }):
            # Reimportar o módulo após o patching
            with patch('tech.infra.factories.user_gateway_factory.CircuitBreakerUserGateway', circuit_breaker_mock):
                # Act
                gateway = UserGatewayFactory.create(resilience_mode=None)
    ...
    def test_create_with_circuit_breaker_resilience(self, monkeypatch):
        ...
        with patch('tech.infra.factories.user_gateway_factory.CircuitBreakerUserGateway', circuit_breaker_mock):
            # Act
            gateway = UserGatewayFactory.create(resilience_mode='circuit_breaker')
    ...
    def test_create_with_none_resilience(self, monkeypatch):
        ...
        with patch('tech.infra.factories.user_gateway_factory.HttpUserGateway', http_mock):
            # Act
            gateway = UserGatewayFactory.create(resilience_mode='none')
    ...
    def test_create_with_custom_parameters(self, monkeypatch):
        ...
        with patch('tech.infra.factories.user_gateway_factory.CircuitBreakerUserGateway', circuit_breaker_mock):
            # Act
            gateway = UserGatewayFactory.create(
